            ...
            for (int i = 0; i < stocks.Count; i++)
            {
                if (stocks[i].StockId == "2330")
                {
                    ...
                    for (int j  = 0; j < prices.Count; j++)
                    {
                        ...
                        (oneDayBuy, OneDaySell) = history.ParseMainForce(prices[j].StockId, prices[j].Datetime.ToString("yyyy/MM/dd"), prices[j].Datetime.AddDays(-6).ToString("yyyy/MM/dd"));
                        prices[j].五日主力買超張數 = oneDayBuy;
                        prices[j].五日主力賣超張數 = OneDaySell;

                        (oneDayBuy, OneDaySell) = history.ParseMainForce(prices[j].StockId, prices[j].Datetime.ToString("yyyy/MM/dd"), prices[j].Datetime.AddDays(-13).ToString("yyyy/MM/dd"));
                        prices[j].十日主力買超張數 = oneDayBuy;
                        prices[j].十日主力賣超張數 = OneDaySell;

                        (oneDayBuy, OneDaySell) = history.ParseMainForce(prices[j].StockId, prices[j].Datetime.ToString("yyyy/MM/dd"), prices[j].Datetime.AddDays(-27).ToString("yyyy/MM/dd"));
                        prices[j].二十日主力買超張數 = oneDayBuy;
                        prices[j].二十日主力賣超張數 = OneDaySell;

                        (oneDayBuy, OneDaySell) = history.ParseMainForce(prices[j].StockId, prices[j].Datetime.ToString("yyyy/MM/dd"), prices[j].Datetime.AddDays(-55).ToString("yyyy/MM/dd"));
                        prices[j].四十日主力賣超張數 = oneDayBuy;
                        prices[j].四十日主力賣超張數 = OneDaySell;

                        (oneDayBuy, OneDaySell) = history.ParseMainForce(prices[j].StockId, prices[j].Datetime.ToString("yyyy/MM/dd"), prices[j].Datetime.AddDays(-83).ToString("yyyy/MM/dd"));
