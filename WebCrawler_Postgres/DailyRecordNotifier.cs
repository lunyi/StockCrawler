...
namespace WebCrawler
{
    public class DailyRecordNotifier : BaseParser
    {
        ...
        private string Get外資投信主力買超股票(stockContext context)
        {
            var tmpPrice = context.Prices.Where(p =>
        ...
        private string 漲停板(stockContext context, DateTime date)
        {
            var Price = context.Prices.Where(p =>
        ...
        private string 盤整突破(stockContext context, DateTime date)
        {
            var Price = context.Stocks.FromSqlRaw($"exec [usp_GetBreakThrough] '{date:yyyy-MM-dd}'").ToArray();
        ...
        private string 上漲破月線(stockContext context, DateTime date, DateTime preDate)
        {
            var sql = $@"select s.* from Stock s join 
(select * from [Price] a where a.Datetime = '{date:yyyy-MM-dd}') a1 on s.StockId = a1.StockId join 
(select * from [Price] a where a.Datetime = '{preDate:yyyy-MM-dd}') a2 on a1.StockId = a2.StockId
