...
namespace WebCrawler
{
    public class RealtimeChooseParser : BaseParser
    {
        ...
        private async Task InsertAsync(Dictionary<string, Stock[]> newStock)
        {
        ...
        private async Task NotifyBotApiAsync(stockContext context)
        {

            var sql = $@"SELECT  newid() as Id, r1.StockId , r1.Name, r1.Datetime, '突破季線 5與20日均線黃金交叉' as [Type], '' as Description
  FROM  [RealtimeBestStock] r1 
  join [RealtimeBestStock] r2 on r1.StockId = r2.StockId 
  where
  ...
  group by r1.StockId , r1.Name, r1.Datetime";
        ...
        private void ParseWarnStock(stockContext context, IQueryable<RealtimeBestStock> current)
        {
            string type = "警示股";
