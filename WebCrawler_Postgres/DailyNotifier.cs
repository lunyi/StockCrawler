...
namespace WebCrawler
{
    public class DailyNotifier : BaseParser
    {
        ...
        private string 漲停板(stockContext context)
        {
        ...
        {
            var Price = context.Stocks.FromSqlRaw($"exec [usp_GetBreakThrough] '{DateTime.Now:yyyy-MM-dd}'").ToArray();
...
";
        ...
        private string 均線起飛第一天(stockContext context)
        {
            var Price = context.Prices.Where(p =>
        ...
        private async Task<string> GetSqlByChairmanAsync(stockContext context)
        {
