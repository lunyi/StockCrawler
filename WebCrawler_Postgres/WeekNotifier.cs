...
namespace WebCrawler
{
    public class WeekNotifier : BaseParser
    {
        ...
        private string 主力外資融資大買1(stockContext context)
        {
    ...
    .FirstOrDefault();
            ...
            var stocks = context.Stocks.FromSqlRaw($@"exec [usp_GetWeekBestStock] '{datetime2}'");
        ...
        private string 五日漲幅排行榜1(stockContext context)
        {
            context = new stockContext();
            var sql = $@"
...
WITH TOPTEN as (
   SELECT *, ROW_NUMBER() 
    ...
    FROM [Price] where [Datetime]>= DATEADD(DAY, -7 , @date) and [Datetime] <= @date
)
...
";
        ...
        private string 大戶比例增加1(stockContext context)
        {
            context = new stockContext();
            var sql =
$@"
