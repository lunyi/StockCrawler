...
namespace WebCrawler
{
    public class CnyParser : BaseParser
    {
        ...
        private async Task NotifyAsync(stockContext context)
        {
  ...
  left join (select * from [Price] where [Datetime] = '{DateTime.Today:yyyy/MM/dd}') p on s.StockId = p.StockId
  ...
  order by s.StockId";
        ...
        public Price ParserLastDay(Price price, string stockId, string name)
        {
            ...
            catch (Exception ex)
            {
                ...
                ErrorStock.AddOrUpdate(stockId, name, (key, oldValue) => name);
                return null;
            }
        }
        ...
        public CnyParser()
        {
            ErrorStock = new ConcurrentDictionary<string, string>();
        }
        ...
        private async Task ParseMainForce(stockContext context, string stockId, string name, string datetime)
        {
            var s = Stopwatch.StartNew();
            s.Start();
